...
function StarRating({
    ...
    return <div style={containerStyle} className={className}>
        ...
        <p style={textStyle}>
           {messages.length === starNum ? messages[tempRating ? tempRating -1: starRating - 1]: tempRating || starRating ||  ""}
        </p>
    </div>
}
...
function Star({handleRating, full, onHoverIn, onHoverOut, color, size}) {
    ...
    return <span style={starStyle} onClick={handleRating} onMouseEnter={onHoverIn} onMouseLeave={onHoverOut}>
        {
            full ? 
            <svg
